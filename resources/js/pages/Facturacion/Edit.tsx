...
import { Barrio, CategoriaPredio, Cliente, type BreadcrumbItem, Predio, CicloFacturacion, Factura } from '@/types';
...
export default function Edit({ factura, clientes: initialClientes, predios: initialPredios, ciclos }: EditProps) {
    ...
    const [searchCliente, setSearchCliente] = useState(factura.predio?.cliente?.nombre || '');
    const [showListCliente, setShowListCliente] = useState(false);
    
    // Estado para búsqueda de predio
    const [searchPredio, setSearchPredio] = useState(factura.predio ? `${factura.predio.matricula_predial} - ${factura.predio.direccion_predio}` : '');
    const [showListPredio, setShowListPredio] = useState(false);

    const { data, setData, put, processing, errors } = useForm({
        cliente_id: factura.cliente_id?.toString() || "",
        predio_id: factura.predio_id?.toString() || "",
        ciclo_id: factura.ciclo_id?.toString() || "",
        fecha_emision: factura.fecha_emision ? new Date(factura.fecha_emision).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
        fecha_vencimiento: factura.fecha_vencimiento ? new Date(factura.fecha_vencimiento).toISOString().split('T')[0] : new Date(Date.now() + 15 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
        concepto: factura.concepto || "Consumo mensual de agua",
        saldo_anterior: factura.saldo_anterior || 0,
        saldo_actual: factura.saldo_actual || 0,
        saldo_conexion: factura.saldo_conexion || 0,
        saldo_reconexion: factura.saldo_reconexion || 0,
    ...
    useEffect(() => {
        if (data.cliente_id) {
            ...
            if (prediosDelCliente.length === 1 && !data.predio_id) {
    ...
    // Cargar predios del cliente actual al inicio
    useEffect(() => {
        if (factura.cliente_id) {
            const prediosDelCliente = predios.filter(p => p.cliente_id.toString() === factura.cliente_id.toString());
            setPrediosFiltrados(prediosDelCliente);
        }
    }, []);
        ...
        <DashboardLayout>
            ...
            <div className="space-y-6 max-w-4xl">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold text-balance">Editar Factura #{factura.id}</h1>
                        <p className="text-muted-foreground">Modifique los datos de la factura</p>
                    </div>
                </div>
                        ...
                        <div className="space-y-6">
                            ...
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>Modifique los datos de la factura</CardDescription>
                                </CardHeader>
                            ...
                            <div className="flex items-center justify-end gap-4">
                                ...
                                >
                                    ...
                                    {processing ? 'Guardando...' : ' Actualizar Factura'}
                                </Button>
                            </div>
                        </div>
                    </form>
